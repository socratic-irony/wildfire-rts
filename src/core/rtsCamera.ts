...
export class RTSCameraController {
  ...
  private onPointerDown = (e: PointerEvent) => {
    if (e.button === 2) this.rotating = true; // right
    if (e.button === 1) this.tilting = true;  // middle
  ...
  private onPointerMove = (e: PointerEvent) => {
    if (!this.rotating && !this.tilting) return;
    ...
    if (this.tilting || (this.rotating && e.shiftKey)) {
  ...
  update(dt: number, move: {left:boolean;right:boolean;up:boolean;down:boolean}) {
    ...
    // Keep rig above terrain (simple collision)
    const clearance = 3;
    const y = this.sample(this.root.position.x, this.root.position.z) + clearance;
    if (this.root.position.y < y) this.root.position.y = y;

    // Keep camera looking at pivot
    this.yaw.position.lerp(this.pivot, 0.15);
  }
}
